...
describe('shadow-aware focus helpers', () => {
  ...
  it('walks from the document when no element is given', async () => {
    await withDocument(outer, () => assert.equal(getActiveElement(), input));
    await withDocument(null, () => assert.equal(getActiveElement(), null));
    await withDocument(null, () => assert.equal(getActiveElement(null), null));
  });
  ...
  it('detects editing surfaces through shadow boundaries', async () => {
    ...
    await withDocument(outer, () => assert.equal(isInputFocused(), true));
    await withDocument(element(), () => assert.equal(isInputFocused(), false));
  });
